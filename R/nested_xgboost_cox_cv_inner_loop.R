...
nested_xgboost_cox_cv_inner_loop <- function(data,
  ...
  mtx_training <- nested_xgboost_cox_cv_model_matrix(df_training, event_incident, event_agediff, vars, metab, taxa, randoms)
  ...
  smoof_fun <- smoof::makeSingleObjectiveFunction(
                        ...
                        fn = function(x) {
                          xgbcv <- xgboost::xgb.cv(params = list(
                                                   ...
                                                   nfold = n_inner_folds,
                                                   stratified = TRUE,
                                                   interaction_constraints = interaction_list,
  ...
  mtx_testing <- nested_xgboost_cox_cv_model_matrix(df_testing, event_incident, event_agediff, vars, metab, taxa, randoms)
  ...
  list(data = df_testing,
       ...
       vars = c(vars, metab, taxa, randoms),
       params = mlr_run[["x"]],
       model = xgb_model)
}
