...
```{r}
df_xgboost <- readr::read_tsv("inst/extdata/xgboost.tsv.gz") %>%
...
```{r}
facet_labels <- c("Gain_full" = "Full model", "Gain_minimal" = "Basic model")
custom_colors <- c("TRUE" = "#000000", "FALSE" = "gray40")
```

```{r}
df_lm_meds <- readr::read_tsv("inst/extdata/df_meds.tsv.gz") %>%
  dplyr::filter(term == "med1") %>%
  dplyr::select(model, estimate, qval)
```

```{r}
df_lm_covs <- readr::read_tsv("inst/extdata/df_meds.tsv.gz") %>%
  dplyr::filter(term != "med1") %>%
...
```{r}
library(patchwork)
{p1 + p2 + plot_layout(ncol = 2, widths = c(9, 0.5)) } %>% 
  mysave("bars_xgboost", width = 6, height = 4) 
```
